...
{
   public class HeavyThrower : GearsBase, IWeaponUser
   {
      private readonly CultureCode _culture;

      public HeavyThrower(TournamentParticipant participant)
      {
      ...
      private Weapons SetWeapons()
      {
         ...
         {
            MainWeapon = new EquipmentElement(Runtime.Equipment.ThrownWeapon.OrderByDescending(n => n.Effectiveness).ToList()[index1]),
            ThrowingWeapon = new EquipmentElement(Runtime.Equipment.ThrownWeapon.OrderByDescending(n => n.Effectiveness).ToList()[index2]),
